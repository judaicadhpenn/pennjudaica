#!/usr/bin/env python3
# ============================================================================
# Penn Judaica Portal — OPenn harvester
# ----------------------------------------------------------------------------
# Regenerates data.js by walking OPenn's STATIC web tree. OPenn is NOT an
# OAI-PMH server and has no query API, so the old Sickle/ListRecords sketch in
# HARVESTING.md harvested nothing. OPenn is a predictable file tree:
#
...
#
# This harvester therefore: (1) reads each configured contents page and pulls
# the (repo, object) pairs, (2) fetches each object's browse page, reconstructs
# the image URLs and reads the metadata, (3) writes data.js in the exact
# `window.PJP = { sources, items, featured, harvestedAt }` shape the front end
# expects. It de-duplicates across pages and FAILS LOUDLY if it harvested zero
# items (so the workflow can never silently publish an empty index).
#
# Dependency: requests  (`pip install requests`). Run: `python3 harvest.py`.
# ============================================================================

import re, sys, json, html, time, datetime, argparse, urllib.parse
import requests
...
# collection keys -> display label + colour. Every `source` a harvested item can
# carry MUST appear here, or the front end (SOURCES[it.source].color) crashes.
SOURCES = {
...
def parse_object(repo, obj, forced_source):
    ...
    return {
        ...
        "collection": SOURCES.get(source, SOURCES["other"])["label"],
        "source": source, "iiif": True,
        "img": pages[0]["thumb"],
...
def write_data_js(items, path="data.js"):
    used = sorted({it["source"] for it in items})
    sources = {k: SOURCES.get(k, SOURCES["other"]) for k in used}
    # featured: a few with the most pages, for variety
    featured = [it["id"] for it in sorted(items, key=lambda x: -len(x["pages"]))[:4]]
    payload = {
        "sources": sources,
        "items": items,
        "featured": featured,
    ...
    # compact JSON (no indentation) — at ~1,700 items the pretty-printed file is
    # multiple MB; compact output is far smaller to download and parse.
    body = json.dumps(payload, ensure_ascii=False, separators=(",", ":"))
    with open(path, "w", encoding="utf-8") as f:
        f.write("/* GENERATED by harvest.py on %s — do not edit by hand. */\n"
                % datetime.date.today().isoformat())
        f.write("window.PJP = %s;\n" % body)
    return len(items)


def main():
    ap = argparse.ArgumentParser(description="Harvest OPenn into data.js")
    ap.add_argument("--out", default="data.js")
    ap.add_argument("--limit", type=int, default=None,
                    help="cap objects harvested per collection (for quick tests)")
    args = ap.parse_args()

    items = harvest(limit_per_collection=args.limit)
    if not items:
        print("ERROR: harvested 0 items — refusing to overwrite data.js.", file=sys.stderr)
        sys.exit(1)
    n = write_data_js(items, args.out)
    print("Wrote %s with %d items across %d collections."
          % (args.out, n, len({it['source'] for it in items})), file=sys.stderr)
